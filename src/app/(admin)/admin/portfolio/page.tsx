  ...
  return (
    <div className="space-y-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">จัดการผลงาน (Portfolio)</h1>
          <p className="text-xs text-slate-400 font-light mt-1">
            สร้าง แก้ไข เผยแพร่ หรือลบข้อมูลตัวอย่างผลงานการผลิตสินค้าของคุณ
          </p>
        </div>
      ...
      {projectItems.length === 0 ? (
        <div className="border border-slate-900 bg-slate-900/30 rounded-xl p-12 text-center flex flex-col items-center justify-center space-y-4">
          ...
          <h3 className="text-sm font-bold text-slate-300">ยังไม่มีตัวอย่างผลงานใดๆ</h3>
          <p className="text-xs text-slate-500 font-light max-w-xs leading-relaxed">
            เริ่มต้นแชร์ผลงานการผลิตหมวก เสื้อยืด เสื้อโปโล หรือเครื่องแบบพนักงาน เพื่อช่วยโปรโมตธุรกิจและกระตุ้นการสั่งซื้อ
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-900 bg-slate-900/40 text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                  <th className="py-4 px-6">รูปหลัก</th>
              ...
              <tbody className="divide-y divide-slate-900 text-xs">
                {projectItems.map((project) => (
                  <tr key={project.id} className="hover:bg-slate-900/20 text-slate-300 transition-colors">
