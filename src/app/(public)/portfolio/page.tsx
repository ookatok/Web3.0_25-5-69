/**
 * @file page.tsx
 * @path src/app/(public)/portfolio/page.tsx
 * @description หน้ารวมภาพโครงการผลงานทั้งหมดของบริษัท แยกตามหมวดหมู่ประเภทสินค้าเพื่อดูแนวทางงานผลิต
 */

import React from "react";
import Link from "next/link";
import { Search, Briefcase, Folder, ArrowRight, User } from "lucide-react";
import { container } from "@/infrastructure/di/container";
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from "@/presentation/components/ui/card";
import { Button } from "@/presentation/components/ui/button";
import { cookies } from "next/headers";
import { translations } from "@/shared/i18n/translations";
import Image from "next/image";

interface PortfolioPageProps {
  searchParams: Promise<{
    search?: string;
    category?: string;
    page?: string;
  }>;
}

export const metadata = {
  title: "แฟ้มสะสมผลงาน (Portfolio) | Web3.0 Premium Production",
  description: "ตัวอย่างผลงานสั่งผลิตเสื้อยืด เสื้อโปโล หมวกยูนิฟอร์มพนักงานองค์กรคุณภาพเกรดพรีเมียม",
};

export default async function PublicPortfolioPage({ searchParams }: PortfolioPageProps) {
  const resolvedSearchParams = await searchParams;
  const search = resolvedSearchParams.search || "";
  const category = resolvedSearchParams.category || "";
  const currentPage = Math.max(1, parseInt(resolvedSearchParams.page || "1", 10));
  const limit = 9;
  const offset = (currentPage - 1) * limit;

  const cookieStore = await cookies();
  const lang = (cookieStore.get("lang")?.value || "th") as "th" | "en";
  const t = translations[lang];

  // Fetch projects from usecase
  const { projects: projectItems, total } = await container.listProjects.execute({
    limit,
    offset,
    status: "PUBLISHED",
    search: search || undefined,
    category: category || undefined,
  });

  const totalPages = Math.ceil(total / limit);

  // Match presets in select options
  const defaultOption = lang === "th" ? "ทั้งหมด" : "All";
  const categories = lang === "th"
    ? ["ทั้งหมด", "เสื้อยืด", "เสื้อโปโล", "หมวก", "ชุดยูนิฟอร์ม", "สินค้าพรีเมียม"]
    : ["All", "T-Shirt", "Polo Shirt", "Cap", "Uniform", "Premium Gift"];

  const getCategoryDbValue = (cat: string) => {
    switch (cat) {
      case "All":
      case "ทั้งหมด":
        return "";
      case "T-Shirt":
      case "เสื้อยืด":
        return "เสื้อยืด";
      case "Polo Shirt":
      case "เสื้อโปโล":
        return "เสื้อโปโล";
      case "Cap":
      case "หมวก":
        return "หมวก";
      case "Uniform":
      case "ชุดยูนิฟอร์ม":
        return "ชุดยูนิฟอร์ม";
      case "Premium Gift":
      case "สินค้าพรีเมียม":
        return "สินค้าพรีเมียม";
      default:
        return cat;
    }
  };

  return (
    <div className="min-h-screen bg-theme-bg py-24 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center font-sans space-y-8">
      {/* Header Card */}
      <div className="w-full max-w-5xl bg-theme-card-bg text-theme-card-text rounded-[2.5rem] p-8 md:p-12 border-[4px] border-theme-card-border shadow-2xl relative overflow-hidden">
        {/* Banner Background Image Overlay */}
        <div className="absolute inset-0 w-full h-full pointer-events-none select-none z-0">
          <Image
            src="/portfolio_banner_bg.png"
            alt="Portfolio Production Background"
            fill
            sizes="100vw"
            priority
            className="object-cover opacity-[0.06] dark:opacity-[0.12] mix-blend-luminosity"
          />
        </div>
        <div className="space-y-4 relative z-10">
          <span className="font-mono text-[10px] tracking-widest text-theme-card-subtext uppercase font-bold">{t.portSub}</span>
          <h1 className="font-teko text-5xl sm:text-7xl md:text-8xl font-bold uppercase tracking-wider leading-none">
            {t.portTitle}
          </h1>
          <p className="font-mono text-xs text-theme-card-subtext uppercase tracking-widest">
            {t.portDescSub}
          </p>
          <p className="text-xs text-theme-card-subtext max-w-lg leading-relaxed font-light">
            {t.portDescText}
          </p>
        </div>
      </div>

      {/* Filter & Search Bar Card */}
      <div className="w-full max-w-5xl bg-theme-card-bg border-[4px] border-theme-card-border rounded-[2rem] p-6 text-theme-card-text flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
        {/* Categories Tab */}
        <div className="flex overflow-x-auto whitespace-nowrap scrollbar-none gap-2 w-full md:w-auto font-mono pb-2 md:pb-0">
          {categories.map((cat) => {
            const isDefault = cat === defaultOption;
            const dbVal = getCategoryDbValue(cat);
            const isActive = (isDefault && !category) || category === dbVal;
            const href = isDefault 
              ? `/portfolio${search ? `?search=${search}` : ""}`
              : `/portfolio?category=${dbVal}${search ? `&search=${search}` : ""}`;

            return (
              <Link
                key={cat}
                href={href}
                className={`px-4 py-1.5 rounded-full text-[9px] font-bold tracking-widest uppercase transition-all duration-200 border ${
                  isActive
                    ? "bg-theme-inverted-bg border-theme-inverted-border text-theme-inverted-text font-extrabold"
                    : "bg-theme-bg text-theme-card-subtext border-transparent hover:text-theme-card-text hover:border-theme-card-border"
                }`}
              >
                {cat}
              </Link>
            );
          })}
        </div>

        {/* Search Form */}
        <form action="/portfolio" method="GET" className="relative w-full md:w-80">
          {category && <input type="hidden" name="category" value={category} />}
          <input
            type="text"
            name="search"
            placeholder={t.portSearch}
            defaultValue={search}
            className="w-full bg-theme-bg border border-theme-card-border rounded-full px-5 py-2.5 pl-10 text-[10px] text-theme-card-text placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-theme-card-text focus:border-theme-card-text transition-all font-mono font-bold tracking-wider uppercase"
          />
          <Search className="absolute left-4 top-3 w-3.5 h-3.5 text-theme-card-subtext" />
        </form>
      </div>

      {/* Projects Grid Card */}
      <div className="w-full max-w-5xl bg-[#c2c4c6] text-[#131415] rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-12 border-[4px] border-theme-card-border shadow-2xl space-y-8">
        {projectItems.length === 0 ? (
          <div className="text-center py-20 bg-theme-bg text-theme-card-text border border-theme-card-border rounded-[2rem] space-y-4">
            <p className="font-mono text-xs text-theme-card-subtext uppercase tracking-widest">{t.portNoResult}</p>
            {(search || category) && (
              <Link href="/portfolio">
                <Button className="rounded-full bg-theme-button-primary-bg text-theme-button-primary-text hover:bg-theme-button-primary-bg/85 font-mono tracking-widest text-[9px] uppercase py-4 px-6">
                  {t.blogClear}
                </Button>
              </Link>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projectItems.map((project) => (
              <Card 
                key={project.id} 
                className="group flex flex-col bg-theme-card-bg text-theme-card-text border-2 border-transparent hover:border-theme-card-text rounded-[2rem] overflow-hidden shadow-lg transition-all duration-300"
              >
                {/* Cover Image inside card with padding */}
                <div className="relative h-72 m-3 bg-theme-card-bg rounded-[1.5rem] overflow-hidden border border-theme-card-border">
                  {project.coverImage ? (
                    <Image
                      src={project.coverImage}
                      alt={project.title}
                      fill
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw"
                      unoptimized={project.coverImage.startsWith("http")}
                      className="object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-950 via-indigo-950/20 to-purple-950/20">
                      <Briefcase className="w-8 h-8 text-indigo-500/30" />
                    </div>
                  )}
                  {project.category && (
                    <span className="absolute top-3 left-3 bg-black/75 backdrop-blur-md text-[8px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border border-white/10 text-white flex items-center gap-1">
                      <Folder className="w-3 h-3" />
                      {project.category}
                    </span>
                  )}
                </div>

                <CardHeader className="space-y-2 p-5 pb-2">
                  {project.client && (
                    <div className="flex items-center gap-1 text-[8px] font-mono text-theme-card-subtext uppercase tracking-widest font-bold">
                      <User className="w-3 h-3 text-theme-card-subtext" />
                      <span>{lang === "th" ? "ลูกค้า:" : "Client:"} {project.client}</span>
                    </div>
                  )}
                  <CardTitle className="font-mono text-xs font-bold text-theme-card-text group-hover:text-theme-card-subtext transition-colors line-clamp-2 leading-snug uppercase">
                    {project.title}
                  </CardTitle>
                </CardHeader>

                <CardContent className="flex-grow p-5 pt-0 pb-4">
                  <p className="text-[11px] text-theme-card-subtext font-light leading-relaxed line-clamp-3">
                    {project.description}
                  </p>
                </CardContent>

                <CardFooter className="p-5 pt-0 border-t border-theme-card-border">
                  <Link 
                    href={`/portfolio/${project.slug}`} 
                    className="w-full inline-flex items-center justify-between text-[9px] font-mono tracking-widest text-theme-card-subtext group-hover:text-theme-card-text uppercase pt-4"
                  >
                    <span>{lang === "th" ? "ดูรายละเอียด" : "VIEW DETAILS"}</span>
                    <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </CardFooter>
              </Card>
            ))}
          </div>
        )}

        {/* Pagination */}
        {totalPages > 1 && (
          <div className="flex justify-center gap-2 pt-6 font-mono">
            <Link
              href={`/portfolio?page=${currentPage - 1}${category ? `&category=${category}` : ""}${
                search ? `&search=${search}` : ""
              }`}
              className={`px-4 py-2 rounded-full border text-[9px] tracking-widest uppercase transition-colors ${
                currentPage === 1
                  ? "opacity-50 pointer-events-none text-theme-card-subtext border-theme-card-border"
                  : "bg-white border-white text-black font-bold hover:bg-slate-200"
              }`}
            >
              {lang === "th" ? "ก่อนหน้า" : "PREV"}
            </Link>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <Link
                key={p}
                href={`/portfolio?page=${p}${category ? `&category=${category}` : ""}${
                  search ? `&search=${search}` : ""
                }`}
                className={`w-8 h-8 flex items-center justify-center rounded-full text-[9px] font-bold border transition-all ${
                  currentPage === p
                    ? "bg-theme-button-primary-bg border-theme-button-primary-bg text-theme-button-primary-text"
                    : "bg-theme-bg border-transparent text-theme-card-subtext hover:border-black hover:text-black"
                }`}
              >
                {p}
              </Link>
            ))}
            <Link
              href={`/portfolio?page=${currentPage + 1}${category ? `&category=${category}` : ""}${
                search ? `&search=${search}` : ""
              }`}
              className={`px-4 py-2 rounded-full border text-[9px] tracking-widest uppercase transition-colors ${
                currentPage === totalPages
                  ? "opacity-50 pointer-events-none text-theme-card-subtext border-theme-card-border"
                  : "bg-white border-white text-black font-bold hover:bg-slate-200"
              }`}
            >
              {lang === "th" ? "ถัดไป" : "NEXT"}
            </Link>
          </div>
        )}
      </div>
    </div>
  );
}
