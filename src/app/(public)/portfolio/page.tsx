...
export default async function PublicPortfolioPage({ searchParams }: PortfolioPageProps) {
  ...
  const categories = ["ทั้งหมด", "เสื้อยืด", "เสื้อโปโล", "หมวก", "ชุดยูนิฟอร์ม", "สินค้าพรีเมียม"];
    ...
    <div className="min-h-screen bg-[#131415] py-24 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center font-sans space-y-8">
      ...
      <div className="w-full max-w-5xl bg-[#212224] text-white rounded-[2.5rem] p-8 md:p-12 border-[4px] border-[#2c2d30] shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-4 relative z-10">
          <span className="font-mono text-[10px] tracking-widest text-slate-400 uppercase font-bold">Our Showcase</span>
          ...
          <p className="text-xs text-slate-400 max-w-lg leading-relaxed font-light">
            สัมผัสคุณภาพงานตัดเย็บ งานปักคอมพิวเตอร์ และงานสกรีนจากตัวอย่างผลงานการสั่งผลิตจริงของลูกค้าองค์กรและแบรนด์แฟชั่น
          </p>
        </div>
      </div>
      ...
      <div className="w-full max-w-5xl bg-[#2d2e30] border-[4px] border-[#202225] rounded-[2rem] p-6 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto font-mono">
          {categories.map((cat) => {
            ...
            const href = cat === "ทั้งหมด" 
              ...
              : `/portfolio?category=${cat}${search ? `&search=${search}` : ""}`;
        ...
        <form action="/portfolio" method="GET" className="relative w-full md:w-80">
          ...
          />
          <Search className="absolute left-4 top-3 w-3.5 h-3.5 text-slate-500" />
        </form>
      </div>
      ...
      <div className="w-full max-w-5xl bg-[#c2c4c6] text-[#131415] rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-12 border-[4px] border-[#202225] shadow-2xl space-y-8">
        {projectItems.length === 0 ? (
          <div className="text-center py-20 bg-[#1d1f22] text-white border border-[#333] rounded-[2rem] space-y-4">
            <p className="font-mono text-xs text-slate-400 uppercase tracking-widest">No projects found in this category</p>
            {(search || category) && (
              <Link href="/portfolio">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projectItems.map((project) => (
              ...
              >
                ...
                <div className="relative h-56 m-3 bg-[#2d2e30] rounded-[1.5rem] overflow-hidden border border-white/5">
                  {project.coverImage ? (
                ...
                <CardHeader className="space-y-2 p-5 pb-2">
                  {project.client && (
                    <div className="flex items-center gap-1 text-[8px] font-mono text-slate-500 uppercase tracking-widest font-bold">
